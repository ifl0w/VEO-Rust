...
``cargo run --release --example octree --features vulkan``
...
Every object in the scene is an entity that is basically an empty container and aggregates components.
...
The actual rendering is implemented with the low level and unsafe rust library gfx-hal, which is modeled closely after Vulkan specification.
The necessary information is extracted from the components and used to record the command buffers.
Furthermore, objects that share visible properties and can be rendered are identified and rendered with instancing if possible.
...
This is equivalent to 8^10 (ca. 1.07E9) nodes.
...
The first stage traverses the complete octree and always renders the maximum number of allowed nodes.
This results in a frame time in the order of 1 second.
...
The number of rendered nodes is about 1 million and the framerate is in the order of 0.1 seconds.
...
In the case of the scene used, this results in the largest number of nodes when the camera plane cuts through the middle of level.
...
This shows that the optimizations have no overlapping effect and that their combination can be considered very effective.
The frametime drops to the order of 0.01 seconds (100 fps) and therefore can be considered real time.
...
It would be necessary to implement an efficient data stream of the octree data to the GPU since VRAM typically is smaller than the system memory and large octrees would not fit on the GPU.
...
If a volume is present, the number of nodes can be significantly larger.
...
The evaluation of the implemented optimizations shows their effectiveness clearly.
Both optimizations, frustum culling and depth limitation, reduce the complexity by a factor of 10.
They do not overlap and can, therefore, be combined to achieve a complexity reduction of a factor of 100.
